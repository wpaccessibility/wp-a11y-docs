---
title: Test for accessibility
layout: default
nav_order: 4
meta_data: false
description: Find ways to test for accessibility yourself.
---

# Test for accessibility

How to test your work and check for accessibility issues? How to test for the WCAG guidelines and for best practices?

In the pages of this section, you can find several ways to test accessibility for different topics and workflows.

How to test for `accessibility-ready` is described with the requirements in the section [Theme guidelines for the WordPress accessibility-ready program](/docs/accessibility-ready/theme-guidelines/).
